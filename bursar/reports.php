...
<head>
	<meta charset="UTF-8">
	<title>Bursar Dashboard - RHMS</title>
	 ...
	 ?>
		...
		<div class="statistics">
			...
			$year = date('Y');
				...
				$result = mysqli_query($con,$sql);
				while ($row = mysqli_fetch_array($result)) {
					$sql1 = mysqli_query("SELECT sum(test_price) AS bb FROM `medication` WHERE `month`='$month' AND `year`='$year'");
					while ($row1 = mysqli_fetch_array($sql1)) {
						$sql2 =mysqli_query("SELECT sum(medical_price) AS cc FROM `medication` WHERE `month`='$month' AND `year`='$year'");
						while ($row2 = mysqli_fetch_assoc($sql2)) {
							$all = $row['aa'] + $row1['bb'] + $row2['cc'];
							echo number_format($all);
						}
			...
			$con=mysqli_connect("localhost","root","","hospital");
				...
				while ($row = mysqli_fetch_array($con,$result)) {
					$sql1 = mysqli_query("SELECT sum(test_price) AS bb FROM `medication` WHERE `year`='$year'");
					while ($row1 = mysqli_fetch_array($sql1)) {
						$sql2 =mysqli_query("SELECT sum(medical_price) AS cc FROM `medication` WHERE `year`='$year'");
						while ($row2 = mysqli_fetch_assoc($sql2)) {
							$all = $row['aa'] + $row1['bb'] + $row2['cc'];
							echo number_format($all);
						}
